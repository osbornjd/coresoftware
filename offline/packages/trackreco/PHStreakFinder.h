...
typedef std::pair<point, int> pointKey;
typedef std::pair<point, float> stubKey;
typedef uint64_t cluskey;




class PHStreakFinder : public SubsysReco
{
 public:
  PHStreakFinder(const std::string &name = "PHStreakFinder",const std::string &filename = "streakntuple.root");

  double chisq(const double *xx);
  //vector<TrkrCluster*> clusterpoints;
  virtual ~PHStreakFinder()
  {
  }
 ...
 private:
  ...
  //double chisq(const double *xx);
  void get_stub(const bgi::rtree<pointKey, bgi::quadratic<16>> &rtree, float pointx, float pointy, float pointz, int &count, double &xzslope, double &xzintercept, double &yzslope, double &yzintercept);
 ...
 private:
  ...
  TNtuple *_ntp_ev = nullptr;
  TNtuple *_ntp_clu = nullptr;
  TNtuple *_ntp_stub = nullptr;
  TNtuple *_ntp_trk = nullptr;
  TNtuple *_ntp_clutrk = nullptr;
