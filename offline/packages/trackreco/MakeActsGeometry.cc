...
void MakeActsGeometry::buildActsSurfaces()
{
  ...
  rescaleStr.str("");
  rescaleStr << m_magFieldRescale;

  if(Verbosity() > -1)
    std::cout << "Mag field now " << m_magField << " with rescale "
	      << rescaleStr.str().c_str() << std::endl;

  std::string responseFile, materialFile;
  setMaterialResponseFiles(responseFile, materialFile);
...
void MakeActsGeometry::setMaterialResponseFiles(std::string& responseFile,
